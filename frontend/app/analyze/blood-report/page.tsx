  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      while (renderedHeight < canvas.height) {
        ...
        pdf.addImage(
          `data:image/svg+xml;utf8,<svg width='32' height='32' xmlns='http://www.w3.org/2000/svg'><circle cx='16' cy='16' r='15' fill='%23b91c1c' stroke='white' stroke-width='2'/><text x='16' y='22' font-size='16' font-family='Arial' fill='white' text-anchor='middle' font-weight='bold'>AI</text></svg>`,
          'PNG',
          margin,
          margin,
          24,
        ...
        pdf.setFontSize(15);
        pdf.text('Blood Report Analysis', margin + 32, margin + 16, { baseline: 'middle' });
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(10);
        pdf.text(exportDateString, pageWidth - margin - pdf.getTextWidth(exportDateString), margin + 10);
        pdf.setFontSize(11);
        if (chatTitle) {
          pdf.text(`Patient: ${chatTitle}`, margin + 32, margin + 28);
        }
        // Draw image below header
        const imageY = margin + headerHeight;
        pdf.addImage(imgData, 'PNG', margin, imageY, imgWidth, pageImgHeight);
        // Draw watermark (app name) diagonally
        pdf.setTextColor(220, 220, 220);
        pdf.setFontSize(40);
        pdf.text('GeneticAI', pageWidth / 2, pageHeight / 2, { angle: 30, align: 'center' });
        ...
        <div ref={resultRef} className="mt-10 bg-medical-50 dark:bg-bluegray-800 rounded-xl shadow p-6">
          <div className="flex justify-end mb-4">
            <span className="relative group">
              ...
              <span className="absolute z-10 left-1/2 -translate-x-1/2 mt-2 px-3 py-1 rounded bg-gray-900 text-white text-xs opacity-0 group-hover:opacity-100 transition pointer-events-none whitespace-nowrap shadow-lg">
                Download the analysis as a PDF (with formatting)
              </span>
            </span>
          </div>
